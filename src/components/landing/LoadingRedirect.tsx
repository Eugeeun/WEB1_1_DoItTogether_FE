import ServiceTitle from '@/components/landing/ServiceTitle';
...
export default function LoadingRedirect({ handleClose }: LoadingRedirectProps) {
  ...
  return (
    <div className='fixed inset-0 z-[9999] flex flex-col items-center justify-between bg-white p-5 text-center'>
      <ServiceTitle />
      <div className='w-full'>
        <p className='mb-2 leading-relaxed text-gray2 font-subhead'>
          더 나은 서비스 이용을 위해
          <br />
          외부 브라우저로 이동하세요
        </p>
        ...
        <button
          ...
          className='mb-6 w-full rounded-lg bg-main px-6 py-3 text-white shadow-md font-subhead'
        >
          외부 브라우저 바로가기
        </button>
      ...
      <button onClick={handleClose} className='mt-2 text-gray6 underline font-body'>
        닫기
      </button>
    </div>
  );
